...
namespace Mirror
{
    ...
    public class SubTelepathyTransport : Transport, PortTransport
    {
        ...
        SubTelepathy.SubClient client;
        SubTelepathy.SubServer server;
        ...
        void Awake()
        {
            ...
            SubTelepathy.Log.Info = Debug.Log;
            SubTelepathy.Log.Warning = Debug.LogWarning;
            SubTelepathy.Log.Error = Debug.LogError;

            // allocate enabled check only once
            enabledCheck = () => enabled;

            Debug.Log("SubTelepathyTransport initialized!");
        }
        ...
        {
            // create client
            client = new SubTelepathy.SubClient(clientMaxMessageSize);
            // client hooks
        ...
        {
            // create server
            server = new SubTelepathy.SubServer(serverMaxMessageSize);
        ...
        {
            Debug.Log("SubTelepathyTransport Shutdown()");
            client?.Disconnect();
            client = null;
            server?.Stop();
            server = null;
        ...
        public override string ToString() => $"SubTelepathy [{port}]";
    }
}
