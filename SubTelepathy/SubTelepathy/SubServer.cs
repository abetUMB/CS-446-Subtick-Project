...
namespace SubTelepathy
{
    public class SubServer : Common
    {
        // events to hook into
        ...
        // constructor
        public SubServer(int MaxMessageSize) : base(MaxMessageSize) {}
        ...
        void Listen(int port)
        {
            ...
            try
            {
                ...
                listener.Start();
                SubLog.Info("Server: listening port=" + port);
                ...
                while (true)
                {
                    ...
                    Thread sendThread = new Thread(() =>
                    {
                        ...
                        {
                            SubLog.Error("Server send thread exception: " + exception);
                        }
                    });
                    ...
                    Thread receiveThread = new Thread(() =>
                    {
                        ...
                        {
                            SubLog.Error("Server client thread exception: " + exception);
                        }
                    });
            ...
            catch (ThreadAbortException exception)
            {
                ...
                SubLog.Info("Server thread aborted. That's okay. " + exception);
            }
            catch (SocketException exception)
            {
                // calling StopServer will interrupt this thread with a
                // 'SocketException: interrupted'. that's okay.
                SubLog.Info("Server Thread stopped. That's okay. " + exception);
            }
            catch (Exception exception)
            {
                // something went wrong. probably important.
                SubLog.Error("Server Exception: " + exception);
            }
        }
        ...
        public bool Start(int port)
        {
            ...
            SubLog.Info("Server: Start port=" + port);
        ...
        public bool Send(int connectionId, ArraySegment<byte> message)
        {
            ...
            if (message.Count <= MaxMessageSize)
            {
                ...
                if (clients.TryGetValue(connectionId, out ConnectionState connection))
                {
                    ...
                    {
                        // log the reason
                        SubLog.Warning($"Server.Send: sendPipe for connection {connectionId} reached limit of {SendQueueLimit}. This can happen if we call send faster than the network can process messages. Disconnecting this connection for load balancing.");
            ...
            SubLog.Error("Server.Send: message too big: " + message.Count + ". Limit: " + MaxMessageSize);
            return false;
        }
        ...
        public bool Disconnect(int connectionId)
        {
            ...
            if (clients.TryGetValue(connectionId, out ConnectionState connection))
            {
                // just close it. send thread will take care of the rest.
                connection.client.Close();
                SubLog.Info("Server.Disconnect connectionId:" + connectionId);
                return true;
            }
            return false;
        }
