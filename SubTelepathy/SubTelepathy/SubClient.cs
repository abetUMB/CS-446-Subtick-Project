...
namespace SubTelepathy
{
    ...
    public class SubClient : Common
    {
        ...
        // constructor
        public SubClient(int MaxMessageSize) : base(MaxMessageSize) {}
...

        {
            ...
            catch (SocketException exception)
            {
                ...
                SubLog.Info("Client Recv: failed to connect to ip=" + ip + " port=" + port + " reason=" + exception);
            }
            ...
            catch (Exception exception)
            {
                ...
                SubLog.Error("Client Recv Exception: " + exception);
            }
        ...
        public void Connect(string ip, int port)
        {
            ...
            {
                SubLog.Warning("SubTelepathy Client can not create connection because an existing connection is connecting or connected");
                return;
            }
            ...
            //   addresses but only connects to IPv6 servers (e.g. SubTelepathy).
        ...
        public bool Send(ArraySegment<byte> message)
        {
            if (Connected)
            {
                ...
                if (message.Count <= MaxMessageSize)
                {
                    ...
                    {
                        // log the reason
                        SubLog.Warning($"Client.Send: sendPipe reached limit of {SendQueueLimit}. This can happen if we call send faster than the network can process messages. Disconnecting to avoid ever growing memory & latency.");
                ...
                SubLog.Error("Client.Send: message too big: " + message.Count + ". Limit: " + MaxMessageSize);
                return false;
            }
            SubLog.Warning("Client.Send: not connected!");
            return false;
        }
